...
namespace Yootek.Authorization.Users
{
    public class UserManager : AbpUserManager<Role, User>
    {
        ...
        public async Task<List<UserIdentifier>> GetUsersOrganizationUnitByType(APP_ORGANIZATION_TYPE type, long? parentId = null, long? organizationUnitId = null)
        {
            try
            {
                using (_unitOfWorkManager.Current.SetTenantId(AbpSession.TenantId))
                {
                    var parent = await _organizationUnitRepository.FirstOrDefaultAsync(parentId ?? 0);
                    var handleOU =  await _organizationUnitRepository.FirstOrDefaultAsync(organizationUnitId ?? 0);
                    var ids = await _organizationUnitRepository.GetAll()
                        .Where(x => x.Type == type)
                        .WhereIf(parentId.HasValue && parent != null, x => x.Code.StartsWith(parent.Code))
                        .WhereIf(organizationUnitId.HasValue && organizationUnitId != null, x => x.Code.StartsWith(handleOU.Code))
                        .Select(x => x.ParentId ?? 0)
                        .Distinct()
                        .ToListAsync();
                    var users = await _userOrganizationUnitRepository.GetAll().
        ...
        public  async Task<List<UserIdentifier>> GetAllUserByOrganizationUnit(long organizationUnitId)
        {
            var org = await _organizationUnitRepository.FirstOrDefaultAsync(organizationUnitId);
            if (org == null) return new List<UserIdentifier>();
            var ids = await _organizationUnitRepository.GetAll()
               .Where(x => x.Code.StartsWith(org.Code)
               && x.Type != APP_ORGANIZATION_TYPE.BUILDING
               && x.Type != APP_ORGANIZATION_TYPE.URBAN
               && x.Type != APP_ORGANIZATION_TYPE.REPRESENTATIVE_NAME)
               .Select(x => x.ParentId)
               .Distinct()
               .ToListAsync();
            ...
            var query = from ouUser in _userOrganizationUnitRepository.GetAll()
                        ...
                        join user in Users on ouUser.UserId equals user.Id
                        select new
                        {
                            User = user,
            ...
            var result = await query.Where(x => ids.Contains(x.OrganizationUnitId)).Select(x => x.User.ToUserIdentifier()).Distinct().ToListAsync();
            return result;
        }

        public async Task<List<UserIdentifier>> GetAllUserByDepartment(long departmentId)
        {
            var users = await _deptUserRepo.GetAll().Where(x => x.DeptId == departmentId).Select(x => new UserIdentifier(x.TenantId, x.UserId)).ToListAsync();
            return users;
        }
        #endregion
